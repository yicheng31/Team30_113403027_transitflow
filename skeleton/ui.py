...
def get_chat_model_choices() -> list:
    ...
    choices = []
    for model in _KNOWN_OLLAMA_MODELS:
        label = model if model in available else f"{model}  (not pulled)"
        choices.append((label, model))
    choices.append((f"Gemini ({GEMINI_CHAT_MODEL})", "gemini"))
    return choices
...
def on_chat_model_change(value: str):
    if value == "gemini":
        ...
        return f"**Active:** Gemini ({GEMINI_CHAT_MODEL})\n\n{status}", get_ollama_status()

    available = set(llm.get_available_ollama_models())
    if value not in available:
        return f"`{value}` is not pulled. Run: `ollama pull {value}`", get_ollama_status()
...
"""
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            with gr.Row():
                clear_btn = gr.Button("清除對話", size="sm")
